...
#lmMod <- lm(loan_amount ~  house_area + annual_income + secondary_business + monthly_expenses +  loan_purpose, data=TrainData )  # build the model

##########################Multinomial Regression################################
lmMod <- lm(loan_amount ~ poly(house_area, degree=3, raw=TRUE) +
     poly(annual_income, degree=3, raw=TRUE) +
     poly(secondary_business, degree=3, raw=TRUE) +
     poly(monthly_expenses, degree=3, raw=TRUE) +
     poly(loan_purpose, degree=3, raw=TRUE))
TestData$pred <- predict(lmMod, TestData)


TestData$pred <- round(pred, digits = -2)
TestData$Error <- (abs(TestData$pred - TestData$loan_amount)/TestData$loan_amount) * 100
TestData$Error[is.na(TestData$Error)]<- 0
duration_quantiles <- data.frame(quantile(TestData$Error,
                                          prob=c(0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,
                                                 0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,
                                                 0.90,0.95,0.99,1)))
summary(lmMod)